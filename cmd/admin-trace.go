...
func writeTracelogsFile(log string) {
	...
	if _, err := traceLogsFile.WriteString(log + "\n"); err != nil {
		traceLogsFile.Close()
		fatalIf(probe.NewError(err), "Unable to write trace log file `"+defineTraceLogsFile()+"`")
	}
}
...
func compressGzip(asynchTraceLogsFile *os.File, asynchTraceLogsFilePath string) {
	// Remove raw file
	defer func() {
		if err := os.Remove(asynchTraceLogsFilePath); err != nil {
			// Ignore "no such file or directory". Another log archive already contains this data
			if errors.Is(err, os.ErrNotExist) {
				return
			}
			fatalIf(probe.NewError(err), "Unable to remove current trace log file `"+asynchTraceLogsFilePath+"`")
		}
	}()
	compressFile, err := os.OpenFile(asynchTraceLogsFilePath+".gz", os.O_CREATE|os.O_RDWR, 0o644)
	if err != nil {
		compressFile.Close()
		asynchTraceLogsFile.Close()
		fatalIf(probe.NewError(err), "Unable to open trace log file archive for gzip `"+asynchTraceLogsFilePath+"`")
	}
	asynchTraceLogsFile.Seek(0, io.SeekStart)
	writer := gzip.NewWriter(compressFile)
	_, err = io.Copy(writer, asynchTraceLogsFile)
	writer.Close()
	compressFile.Close()
	asynchTraceLogsFile.Close()
...
func compressZstd(asynchTraceLogsFile *os.File, asynchTraceLogsFilePath string) {
	...
	compressFile, err := os.OpenFile(asynchTraceLogsFilePath+".zst", os.O_CREATE|os.O_RDWR, 0o644)
	if err != nil {
		compressFile.Close()
		asynchTraceLogsFile.Close()
		fatalIf(probe.NewError(err), "Unable to open trace log file archive for zstd `"+asynchTraceLogsFilePath+"`")
	}
	...
	if traceLogsFileZstdEncoder == nil {
		traceLogsFileZstdEncoder, err = zstd.NewWriter(nil)
		if err != nil {
			traceLogsFileZstdEncoder.Close()
			compressFile.Close()
			asynchTraceLogsFile.Close()
			fatalIf(probe.NewError(err), "Unable to create encoder for zstd `"+asynchTraceLogsFilePath+"`")
		}
	}
	traceLogsFileZstdEncoder.ResetContentSize(compressFile, contentSize)

	_, err = io.Copy(traceLogsFileZstdEncoder, asynchTraceLogsFile)
	traceLogsFileZstdEncoder.Close()
	compressFile.Close()
	asynchTraceLogsFile.Close()
	fatalIf(probe.NewError(err), "Unable to copy encoder for zstd `"+asynchTraceLogsFilePath+"`")
}
...
func defineTraceLogsFile() string {
	...
	if traceLogsDirectoryName == "" {
		defineTraceLogsDirectoryName, err := os.Getwd()
		if err != nil {
			fatalIf(probe.NewError(err), "Unable to deterine current working directory")
		}
		traceLogsDirectoryName = defineTraceLogsDirectoryName
	}
	if traceLogsFileName == "" {
		// Default file name
		traceLogsFileName = "minio.trace." + strconv.Itoa(int(traceLogsStartTime.UnixNano())) + ".log"
	}
	// Create hierarchy of directories if they do not exist
	if err := os.MkdirAll(traceLogsDirectoryName, 0o755); err != nil {
		fatalIf(probe.NewError(err), "Unable to create trace log file directory `"+traceLogsDirectoryName+"`")
	}
